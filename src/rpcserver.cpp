...
{ //  category              name                      actor (function)         okSafeMode threadSafe reqWallet
  //  --------------------- ------------------------  -----------------------  ---------- ---------- ---------
    ...
    { "rawtransactions",    "rpctestyo",              &rpctestyo,              true,      false,      false },
...
CRPCTable::CRPCTable()
{
    ...
    for (vcidx = 0; vcidx < (sizeof(vRPCCommands) / sizeof(vRPCCommands[0])); vcidx++)
    {
		...
		//!@# here create a map from pcmd->name(e.g. listunspent) to pcmd(the address of the column which listunspent resides in the vRPCCommands table
...
{
		//!@# find the address of a command's vRPCCommand column by its method name
...
{
    // Find method
    /* !@# if pcmd == NULL , it means that the Method if not found
    	so if we want to add a rpc call, we need to add a strMethod: to  tableRPC
    */
